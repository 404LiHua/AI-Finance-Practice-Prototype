...
def read_scale_panel(path: Path) -> pd.DataFrame:
    """Read the frozen scale panel without changing its bytes or schema."""
    try:
...
def read_feature_panel(path: Path, label: str) -> pd.DataFrame:
    """Read CSV or Parquet feature panels without altering their content."""
    try:
        return pd.read_parquet(path) if path.suffix.lower() == ".parquet" else pd.read_csv(path, dtype={"stock_code": str})
    except Exception as exc:
        raise RuntimeError(f"{label} panel cannot be read as CSV/Parquet: {exc}") from exc
...
def main() -> None:
    ...
    rg3 = read_feature_panel(rg3_path, "RG3")
    rg2 = read_feature_panel(rg2_path, "RG2")
