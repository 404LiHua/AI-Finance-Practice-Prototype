...
def read_daily(path: Path, code: str, calendar: pd.DatetimeIndex, staging_daily: pd.DataFrame | None = None) -> tuple[pd.DataFrame, str]:
    ...
    # pandas normalizes positional ``usecols`` into source-column order before
    # returning the frame.  Request the four fields in ascending source order
    # and assign the matching names explicitly; requesting [1, 2, 15, 11] and
    # assigning by request order silently swapped qfq_close and adjust_factor,
    # flattening every historical return to zero.
    ...
    raw["close"] = pd.to_numeric(raw["qfq_close"], errors="coerce")
    ...
    if staging_daily is not None:
        ...
        if not extra.empty:
            anchor = pd.to_numeric(raw["close"].iloc[-1], errors="coerce") / max(pd.to_numeric(raw["raw_close"].iloc[-1], errors="coerce") * pd.to_numeric(raw["adjust_factor"].iloc[-1], errors="coerce"), 1e-12)
            extra["close"] = pd.to_numeric(extra["close"], errors="coerce") * pd.to_numeric(extra["adj_factor"], errors="coerce") * anchor
            extra = extra[["trade_date", "close"]]
...
def main() -> None:
    ...
    parser.add_argument("--tushare-staging", type=Path, default=None, help="Optional raw Tushare staging directory")
    ...
    if staging_path is not None:
        ...
        staging_hashes = {"daily": sha256(staging_file), "adj_factor": sha256(factor_file)}
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as pool:
        futures = {pool.submit(read_daily, daily_root / f"{code}.csv", code, calendar, staging_daily): code for code in replay_codes}
    ...
    receipt = {
        ...
        "input_sha256": {
            ...
            "tushare_staging": staging_hashes,
        },
