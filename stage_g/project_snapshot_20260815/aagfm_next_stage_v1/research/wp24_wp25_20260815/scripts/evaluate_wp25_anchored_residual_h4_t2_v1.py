...
"""Independent development evaluator for the sealed WP25 predictions."""
...
def main() -> None:
    ...
    parser.add_argument("--prediction-suffix", default="WP25")
    ...
    decision = {"node_id": "WP25_ANCHORED_RESIDUAL_INDEPENDENT_DEVELOPMENT_EVALUATION", "status": "PASS_DEVELOPMENT_JOINT_GATE" if joint_pass else "FAIL_DEVELOPMENT_JOINT_GATE_RESEARCH_ONLY", "protocol_sha256": sha256(protocol_path), "prediction_seal_manifest_sha256": sha256(manifest_path), "fold_metrics": fold_rows, "gates": gates, "production_kernel": "RG_OBGNET_CONFIRMED_SAFE_V1_1", "production_replacement_allowed": False, "fresh_payloads_opened": False, "screening_read": False, "final_read": False, "future_labels_read": False, "metric_read_once": True, "created_at_utc": datetime.now(timezone.utc).isoformat()}; (output / "WP25_INDEPENDENT_EVALUATION_DECISION.json").write_text(json.dumps(decision, ensure_ascii=False, indent=2, allow_nan=False) + "\n", encoding="utf-8")
    markdown = ["# WP25 锚定残差 H4/T2 开发评价", "", f"状态：`{decision['status']}`", "", "本评价读取的只有冻结开发折的验证标签；未读取未来、FRESH、SCREENING 或 FINAL 标签，未修改生产注册表。", "", "| 折 | MAE | 零预测 MAE | 周 IC | MCC | incumbent MCC | Brier | incumbent Brier |", "|---|---:|---:|---:|---:|---:|---:|---:|"]
    ...
    (output / "WP25_INDEPENDENT_EVALUATION_REPORT.md").write_text("\n".join(markdown) + "\n", encoding="utf-8")
