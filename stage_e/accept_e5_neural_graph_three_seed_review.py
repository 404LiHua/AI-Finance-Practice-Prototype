"""Machine acceptance for the frozen E-5.3 three-seed neural/stock-graph review."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--output", type=Path,
        default=REPO_ROOT / "outputs/stage_e/e5_neural_graph_three_seed_acceptance_v1.json",
    )
    ...
    graph_registry = json.loads((root / "real_stock_graph_registry_three_seed.json").read_text(encoding="utf-8"))
    ...
    checks = {
        ...
        "folds_and_seeds_unchanged": metadata["folds"] == review["folds"] == base["folds"] and metadata["seeds"] == review["seeds"] == base["future_three_seeds"],
        "only_two_remaining_seeds_newly_trained": metadata["new_training_seeds"] == review["new_training_seeds"] == [20260723, 20260724],
        ...
        "thirty_six_run_receipts": len(receipts) == 36 and metadata["run_count"] == 36,
        "no_failures": not failures and metadata["failure_count"] == 0,
        "prediction_contract_complete": len(grid) == 12 and all(row["row_count"] == 1500 and row["missing_key_count"] == 0 and row["extra_key_count"] == 0 for row in grid),
        "independent_loading_within_tolerance": bool((receipts["independent_load_max_abs_difference"] <= 1e-7).all()),
        "timegnn_remains_deterministic": len(timegnn_receipts) == 9 and bool(timegnn_receipts["deterministic_graph_sampling"].all()),
        "real_stock_graph_receipts_complete": len(graph_receipts) == 9 and bool(graph_receipts["real_stock_node_graph"].all()),
        "real_stock_graph_registry_complete": len(graph_registry) == 9 and all(row["node_count"] == 100 and row["self_loop_count"] == 100 for row in graph_registry),
        "stock_order_and_adjacency_unchanged": len({row["stock_order_sha256"] for row in graph_registry}) == 1 and len({row["adjacency_sha256"] for row in graph_registry}) == 1,
        ...
        "artifact_hashes_valid": (
            ...
            and sha256_file(root / "real_stock_graph_registry_three_seed.json") == metadata["artifacts"]["real_stock_graph_registry_three_seed_sha256"]
        ),
    ...
    report = {
        "stage": "E-5.3 neural and real-stock-node graph three-seed acceptance",
